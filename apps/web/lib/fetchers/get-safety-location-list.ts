...
export async function getSafetyLocationList(
  params: SafetyLocationListQueryInput,
) {
  ...
  try {
    const res = await apiFetchServer(
      `${SWR_KEYS.safetyLocationList}?${querySearch.toString()}`,
      {
        method: 'GET',
      },
    );
    ...
    throw new Error('Failed to fetch safety location list');
  }
}
