...
export default function DeleteSafetyLocationDialog() {
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      mutate(SWR_KEYS.safetyLocationDetail(safetyLocationId), null);
      mutate(
      ...
      toast.error('Failed to delete safety location. Please try again.');
    } finally {
      setIsPending(false);
    }
  };
      ...
      <DialogContent className='flex flex-col w-full max-w-full sm:max-w-lg max-h-[85vh] p-0 overflow-hidden gap-0 border-0 [&>button]:text-white [&>button]:hover:text-white [&>button]:opacity-70 [&>button]:hover:opacity-100'>
            ...
            <div className='flex items-start gap-4'>
              ...
              <div className='flex flex-col'>
                ...
                <span className='text-sm text-black opacity-50'>
                  This action cannot be undone. The safety location will be
                  permanently removed from the system and the map.
                </span>
              </div>
            </div>
          </div>
        ...
        <DialogFooter className='grid grid-cols-2  bg-[#F9F9F9] rounded-t-2xl p-4 shrink-0'>
          ...
          >
            ...
            <span>{isPending ? 'DELETING...' : 'DELETE LOCATION'}</span>
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
